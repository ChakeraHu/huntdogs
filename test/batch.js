    ...
    describe('execute', function () {
        it('should execute batch request', function (done) {
            ...
            provider.injectValidation(function (payload) {
                ...
                assert.equal(first.method, 'okc_getBalance');
                assert.deepEqual(first.params, ['0x0000000000000000000000000000000000000000', 'latest']);
                assert.equal(second.method, 'okc_getBalance');
                assert.deepEqual(second.params, ['0x0000000000000000000000000000000000000005', 'latest']);
            });

            var batch = web3.createBatch(); 
            batch.add(web3.okc.getBalance.request('0x0000000000000000000000000000000000000000', 'latest', callback));
            batch.add(web3.okc.getBalance.request('0x0000000000000000000000000000000000000005', 'latest', callback2));
            batch.execute();
        });
        ...
        it('should execute batch request for async properties', function (done) {
            ...
            provider.injectValidation(function (payload) {
                ...
                assert.equal(first.method, 'okc_accounts');
            ...
            var batch = web3.createBatch(); 
            batch.add(web3.okc.getAccounts.request(callback));
        ...
        it('should execute batch request with contract', function (done) {
            ...
            provider.injectValidation(function (payload) {
                ...
                assert.equal(first.method, 'okc_getBalance');
                assert.deepEqual(first.params, ['0x0000000000000000000000000000000000000000', 'latest']);
                assert.equal(second.method, 'okc_call');
                assert.deepEqual(second.params, [{
            ...
            var batch = web3.createBatch(); 
            batch.add(web3.okc.getBalance.request('0x0000000000000000000000000000000000000000', 'latest', callback));
            batch.add(web3.okc.contract(abi).at(address).balance.request(address, callback2));
            provider.injectBatchResults([result, result2]);
            batch.execute();
        });
